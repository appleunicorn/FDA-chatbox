...
st.pyplot()
st.markdown("> 📝 Approvals have been increasing with noticeable year-to-year shifts.")

st.subheader("🏢 Unique Applicants by Year")
df2 = plot_applicants_by_year(db_path=db_path, show=False)
st.pyplot()
st.markdown("> 🧪 The applicant base is diverse but led by a few repeat players.")
...
st.pyplot()
st.markdown("> 🏆 Major players dominate, but smaller firms are emerging.")
